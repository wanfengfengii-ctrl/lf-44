...
export function createInitialCargoState(ship: Ship): Cargo[] {
  return []
}
...
export function simulateCargoOperation(
  ...
  if (operationType === 'load') {
    ...
    if (!deck) {
      return { cargos: currentCargos, balance: calculateBalance(currentCargos, ship), cargo: null }
    }
    ...
    const idx = newCargos.findIndex(c => c.id === cargoTask.cargoId || c.name === cargoTask.cargoName)
    if (idx !== -1) {
...
export function generateBalanceHistory(
  ...
  const initialBalance = calculateBalance([], ship)
  ...
  for (const cargoTask of unloadingOrder) {
    ...
    const tiltDelta = Math.abs(result.balance.leftRightTilt) + Math.abs(result.balance.frontBackTilt)
  ...
  const minStabilityScore = stepSnapshots.length > 0
    ? Math.min(...stepSnapshots.map(s => s.balance.stabilityScore))
    : initialBalance.stabilityScore

  const maxTiltAngle = stepSnapshots.length > 0
    ? Math.max(...stepSnapshots.map(s => Math.max(Math.abs(s.balance.leftRightTilt), Math.abs(s.balance.frontBackTilt))))
    : 0
...
function calculateSafetyScore(
  ...
  const avgStability = snapshots.reduce((sum, s) => sum + s.balance.stabilityScore, 0) / snapshots.length
  const minStability = Math.min(...snapshots.map(s => s.balance.stabilityScore))
  const maxTilt = Math.max(...snapshots.map(s => Math.max(Math.abs(s.balance.leftRightTilt), Math.abs(s.balance.frontBackTilt))))
  ...
  const overloadSteps = snapshots.filter(s => s.balance.holdOverloads.some(h => h.overloadRatio > 1)).length
  if (overloadSteps > 0) {
